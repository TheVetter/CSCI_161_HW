...
//        }
        ...
        //Make matrix
        // get sepertator
        String s = getSeperator();
        //make table header
        sb.append(s + "|       ");
        for (Vertex<V> v : vertices) {
            sb.append(String.format("| %5s ", v.getElement()));
        }
        //make table body
        sb.append("\n");
        sb.append(s);
        for(Vertex<V> v : vertices){
            sb.append(String.format("| %5s ", v.getElement())); // left column 
            //fill body
            for(Vertex<V> v1 : vertices()){
                Edge<E> e = getEdge(v, v1) ;
                if (e != null){
                    sb.append(String.format("| %5d ", e.getElement()));                
                }
                else{
